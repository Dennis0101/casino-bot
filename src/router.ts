...
export async function onButton(i: ButtonInteraction, _client: Client) {
  ...
  if (u.banned)
    return i.reply({ ephemeral: true, content: '⛔ 자기차단 상태입니다.' });
  ...
  if (scope === 'lobby') {
    ...
    if (action === 'daily') {
      ...
      const can =
        !me!.dailyClaimed ||
        now.getTime() - new Date(me!.dailyClaimed).getTime() > 86_400_000;
      if (!can)
        return i.reply({
          ephemeral: true,
          content: '오늘 보너스는 이미 받았습니다.',
        });
      ...
      return i.reply({
        ephemeral: true,
        content: `🪙 일일 보너스 **+${num(bonus)}** 지급!`,
      });
    }

    if (action === 'rank') {
      const top: Array<{ userId: string; profit: bigint | number }> =
  ...
  if (scope === 'bac')   return handleBacButton(i, action, rest);    // ✅ 바카라(실규칙/사이드 지원)
}
